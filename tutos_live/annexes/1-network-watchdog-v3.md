...
Le script (situé dans [live_scripts/network_watchdog.sh](../../live_scripts/network_watchdog.sh)) est un véritable "cerveau" capable de s'adapter à votre environnement. Il détecte automatiquement si vous êtes en **WI-FI** (via `wlo1` et le driver `nl80211`) ou en **FILAIRE** (via `nic1/vmbr0` et le driver `wired`) en analysant votre fichier `/etc/network/interfaces`.
...
1. **Script :** Le script de production est maintenu dans [live_scripts/network_watchdog.sh](../../live_scripts/network_watchdog.sh).
