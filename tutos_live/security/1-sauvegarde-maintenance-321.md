...
### ✅ La Solution
Un **script hebdomadaire** (`maintenance.sh`) qui automatise tout.
...
3. Envoie la dernière version sur Google Drive (via `rclone`).

### A. 🧱 Pré‑requis Matériel
*   **HDD externe :** formaté en Ext4.
*   **Google Drive :** Configuré via `rclone`.
...
3.  Reconfigurez le réseau (voir [Tuto 1](../1-deploiement-proxmox-8021x.md)).
