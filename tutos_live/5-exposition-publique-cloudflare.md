...
**Objectif :** Rendre accessible `sentinel.kpihx-labs.com` (et d'autres) depuis n'importe quel navigateur dans le monde, sans ouvrir de port sur le pare‑feu de l'école, et en sécurisant l'accès via votre compte Google.
...
6.  **Sauvegarde le Token** (C'est la longue chaîne de caractères qui commence par `ey...`). **Ne lance pas la commande**, on va utiliser Docker.
...
**✅ La Solution :**
Récupérez le **Template :** [cloudflared.yaml](https://github.com/kpihx-labs/presentation/blob/main/tutos_live/templates/cloudflared.yaml) et déployez-le via Portainer.
...
3.  Choisis **Google**.
4.  Suivez la procédure donnée par Cloudflare (création d'un projet Google Cloud et d'un ID Client OAuth).
